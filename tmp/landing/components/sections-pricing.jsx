// sections-pricing.jsx — Pricing tiers
    ...
    <section id="pricing" className="section" style={{ paddingBlock: 120 }}>
      <div className="container">
        ...
        <div className="reveal" style={{
          marginTop: 28, display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 1,
          background: 'var(--line-soft)', borderRadius: 14, overflow: 'hidden',
          ...
          {[
            ...
            <div key={k} style={{ background: 'var(--surface)', padding: '18px 20px' }}>
              <div className="mono" style={{ fontSize: 10.5, color: 'var(--muted)', letterSpacing: '0.1em' }}>{k.toUpperCase()}</div>
              <div style={{ fontSize: 13.5, color: 'var(--fg)', marginTop: 4, fontWeight: 500, letterSpacing: '-0.01em' }}>{v}</div>
            </div>
          ))}
        </div>
      </div>
...
function PricingCard({ t, i, f = 1, isAnnual = false }) {
  const hi = t.highlight;
  const fmtEur = (n) => '€' + (n % 1 === 0 ? n : n.toFixed(0));
  ...
  const fmtPct = (n) => (n % 1 === 0 ? n + '%' : n.toFixed(2).replace(/\.?0+$/, '') + '%');
      ...
      <div style={{
        ...
        <div>
          <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', marginBottom: 6 }}>
            <h3 className="h-2" style={{
              margin: 0, fontSize: 26, letterSpacing: '-0.022em',
              color: hi ? 'var(--ink-fg)' : 'var(--fg)',
            }}>{t.name}</h3>
            <span className="mono" style={{
              fontSize: 10.5, letterSpacing: '0.08em',
              color: hi ? 'rgba(255,255,255,0.55)' : 'var(--muted-2)',
            }}>{String(i + 1).padStart(2, '0')} / 03</span>
          </div>
          <p style={{
            margin: 0, fontSize: 14, lineHeight: 1.45,
            color: hi ? 'var(--ink-fg-2)' : 'var(--muted)',
          }}>{t.pitch}</p>
        </div>
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 10, flexWrap: 'wrap' }}>
            <span style={{
              fontSize: 44, fontWeight: 500, lineHeight: 1, letterSpacing: '-0.030em',
            ...
            <span style={{
              fontSize: 13, color: hi ? 'var(--ink-muted)' : 'var(--muted)',
            }}>{t.feeSuffix}</span>
          </div>
          {isAnnual && (
            <div className="mono" style={{
              ...
              marginTop: 6,
            }}>FACTURADO ANUAL · AHORRO 10% EN TODO</div>
          )}
            ...
            {[
              ...
              <div key={k} style={{
                ...
                <div style={{
                  ...
                  fontSize: 14, fontWeight: 500, marginTop: 2, letterSpacing: '-0.01em',
                  color: hi ? 'var(--ink-fg)' : 'var(--fg)',
        ...
        <ul style={{
          ...
          {t.features.map((feat, idx) => (
            <li key={idx} style={{
              ...
              padding: '10px 0',
              borderBottom: idx < t.features.length - 1
...
function PriceCalculator() {
  ...
  // % of reservas that go through Solnow's channels (motor + WhatsApp + OTAs).
  // The rest are walk-ins / phone / paid traffic where no commission applies.
  const [throughSolnow, setThroughSolnow] = useState(75);

  const billable = Math.round(reservas * (throughSolnow / 100));
  const gmv = billable * ticket;                  // GMV processed through Solnow (used for commission)
  const totalRevenue = reservas * ticket;         // Total company revenue (all channels)
  const isAnnual = billing === 'annual';
  ...
  const plans = useMemo(() => {
    ...
    return [calc('starter'), calc('pro'), calc('scale')];
  }, [bases, gmv, convs, factor]);

  const eligiblePlans = plans.filter(p => p.eligible);
  const best = eligiblePlans.reduce((a, b) => (a && a.total <= b.total ? a : b), null);
    ...
    <div className="reveal" style={{
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1.1fr', minHeight: 480 }}>
        ...
        <div style={{
          ...
          <Slider label="% de reservas vía Solnow"  unit=" %"      value={throughSolnow} min={20} max={100} step={5} onChange={setThroughSolnow}
...
function BillingToggle({ value, onChange, discount }) {
...
function RevenueBlock({ best, totalRevenue, fmt }) {
    ...
    <div style={{
      ...
      <div>
        ...
        <div style={{ fontSize: 14, color: 'var(--fg-2)', marginTop: 4, lineHeight: 1.5 }}>
          Solnow representa{' '}
          <strong style={{ color: 'var(--accent)', fontSize: 16 }}>{pct}%</strong>
          {' '}de tu facturación mensual ({fmt(totalRevenue)}/mes · todos los canales).
        </div>
      </div>
      <a href="#cta" className="btn btn-primary" style={{ padding: '11px 16px', fontSize: 13.5 }}>
        Pedir demo
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
      <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between' }}>
        ...
        }}>{value.toLocaleString('es-ES')}{unit}</span>
      </div>
      <div style={{ position: 'relative', height: 28, display: 'flex', alignItems: 'center' }}>
        <div style={{
          position: 'absolute', left: 0, right: 0, height: 4,
          borderRadius: 999, background: 'var(--line)',
        }} />
        <div style={{
      ...
      {hint && (
        <div className="mono" style={{ fontSize: 10.5, color: 'var(--muted)', letterSpacing: '0.02em' }}>{hint}</div>
      )}
    </div>
  );
}

function PlanResult({ p, best, fmt, isAnnual }) {
  const labels = { starter: 'Starter', pro: 'Pro', scale: 'Scale' };
