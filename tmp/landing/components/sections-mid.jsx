// sections-mid.jsx — pillars, for-whom, comparison, partner
  ...
  return (
    <section className="section" style={{ paddingBlock: 120 }}>
      <div className="container">
        ...
        />
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 1, background: 'var(--line-soft)', borderRadius: 18, overflow: 'hidden', border: '1px solid var(--line-soft)' }}>
          {pillars.map((p, i) => (
    ...
    <section id="para-quien" className="section" style={{ paddingBlock: 120 }}>
      <div className="container">
        ...
        <div className="reveal" style={{
          ...
          <div style={{ width: 56, height: 56, borderRadius: 14, background: 'oklch(1 0 0 / 0.04)', border: '1px solid var(--line-soft)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          ...
          <div>
            ...
            <p style={{ margin: 0, color: 'var(--muted)', fontSize: 14.5, maxWidth: '64ch' }}>
              Solnow funciona para operadores donde las motos de agua son el activo principal pero también ofrecen quads, kayaks, paddle o parasailing.
              El sistema gestiona toda la flota como un único inventario.
            </p>
          </div>
        </div>
      </div>
...
function Qualifier({ kind, title, items }) {
...
function Comparison() {
  ...
  const cell = (v, isLast) => {
    if (typeof v === 'string') {
      return <span style={{ fontFamily: 'var(--font-mono)', fontSize: 12, color: isLast ? 'var(--accent)' : 'var(--muted)', letterSpacing: '0.02em' }}>{v}</span>;
    }
      ...
      <div className="container">
              ...
              <div key={c.k} style={{
                ...
                {c.highlight && (
                  <span className="mono" style={{
                    position: 'absolute', top: -1, right: 14, transform: 'translateY(-50%)',
                    fontSize: 9.5, letterSpacing: '0.1em', color: 'oklch(0.12 0.02 240)',
                    background: 'var(--accent)', padding: '3px 7px', borderRadius: 4,
                  }}>NOSOTROS</span>
                )}
                <div style={{ fontSize: 14, fontWeight: 500, color: c.highlight ? 'var(--accent)' : 'var(--fg-2)', letterSpacing: '-0.012em' }}>{c.name}</div>
                <div className="mono" style={{ fontSize: 10.5, color: 'var(--muted-2)', marginTop: 4, letterSpacing: '0.04em' }}>{c.sub}</div>
              </div>
            ))}
          </div>
          {/* rows */}
        ...
        <div className="reveal" style={{ marginTop: 32, display: 'flex', gap: 16, alignItems: 'center', flexWrap: 'wrap' }}>
          <div style={{ display: 'flex', gap: 14, fontSize: 12, color: 'var(--muted)' }}>
            <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6 }}><span style={{ color: 'var(--ok)', fontSize: 14 }}>●</span> incluido</span>
            <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6 }}><span style={{ color: 'var(--warn)' }}>◐</span> parcial</span>
            <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6 }}><span style={{ color: 'var(--muted-2)' }}>○</span> no</span>
          </div>
          <span style={{ flex: 1 }} />
...
Object.assign(window, { Pillars, ForWhom, Comparison, Partner });
